module app
  ...
  real(8), parameter :: cfl    = 5d-1      !CFL CONDITION FOR LIGHT WAVE
  ...
  subroutine set_initial_condition()
    ...
    do j=nys-2,nye+2
       do i=nxgs-2,nxge+2
          ...
          uf(2,i,j) = b0*sin(theta_bn)*sin(phi_bn)
          uf(3,i,j) = b0*sin(theta_bn)*cos(phi_bn)
          uf(4,i,j) = 0.0D0
    ...
    sd(2) = v_the
    do isp=1,nsp
       !$OMP PARALLEL DO PRIVATE(ii,j,gamp)
       do j=nys,nye
          do ii=1,np2(j,isp)
             ...
             gamp = sqrt(1 + (up(3,ii,j,isp)**2 + up(4,ii,j,isp)**2 + up(5,ii,j,isp)**2)/c**2)

             if (up(3,ii,j,isp)*v0 >= 0d0) then
               up(3,ii,j,isp) = (+up(3,ii,j,isp)+v0*gamp)*gam0
             else
               if (uniform_rand() < (-v0*up(3,ii,j,isp)/gamp)) then
                  up(3,ii,j,isp) = (-up(3,ii,j,isp)+v0*gamp)*gam0
               else
                  up(3,ii,j,isp) = (+up(3,ii,j,isp)+v0*gamp)*gam0
               endif
